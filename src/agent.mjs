...
- 「今日のニュース」と聞かれたら、検索クエリに必ず「${jstNow.split('/')[0]}-${jstNow.split('/')[1]}-${jstNow.split('/')[2].split(' ')[0]}」や「${parseInt(jstNow.split('/')[1])}月${parseInt(jstNow.split('/')[2])}日」を含めること。
- **今日の日付以外のニュースは絶対に含めるな。** 検索結果の日付を確認し、今日でないものは除外すること。
...
- 今日の情報が見つからなければ「本日の新着情報は確認できませんでした」と正直に伝えること。`;
